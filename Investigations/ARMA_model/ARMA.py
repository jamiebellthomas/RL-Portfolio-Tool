...
def stationary_check_investigation(asset: Asset):
    """ """
    subsection = asset.extract_subsection(
        asset.time_series, date - datetime.timedelta(days=(round(period * 365))), date
    )
    ...
    plot.add_trace(
        go.Scatter(
            ...
            y=subsection["value"],
            mode="lines",
            name="Original Data (Changing mean and variance)",
        )
    )
    plot.update_layout(
        title="Original Data for " + asset.ticker,
        xaxis_title="Date",
        yaxis_title="Value",
    )
    ...
    plot.add_trace(
        go.Scatter(
            x=subsection.index[1:],
            y=differenced_time_series,
            mode="lines",
            name="Differenced Data (Fixed mean and changing variance)",
        )
    )
    plot.update_layout(
        title="Differenced Data for " + asset.ticker,
        xaxis_title="Date",
        yaxis_title="Value",
    )
    ...
    plot.add_trace(
        go.Scatter(
            x=subsection.index[1:],
            y=log_differenced_time_series,
            mode="lines",
            name="Log Differenced Data (Fixed mean and fixed variance)",
        )
    )
    plot.update_layout(
        title="Log Differenced Data for " + asset.ticker,
        xaxis_title="Date",
        yaxis_title="Value",
    )
...
def ARMA_investigation(asset: Asset):
    ...
    for i in range(index):
        ...
        training_forecast_values.append(current_value)
    training_forecast = pd.Series(
        training_forecast_values, index=training_section.index
    )
    ...
    plot.add_trace(
        go.Scatter(x=forecast.index, y=forecast, mode="lines", name="Forecasted Data")
    )
    # plot training forecast as a dotted line
    plot.add_trace(
        go.Scatter(
            x=training_section.index,
            y=training_forecast,
            mode="lines",
            name="Trained Model",
            line=dict(dash="dash"),
    ...
    plot.update_layout(
        title="Forecasted Data for " + asset.ticker,
        xaxis_title="Date",
        yaxis_title="Value",
    )
    plot.write_image(
        "Investigations/ARMA_model/Forecasted_Data_"
        + asset.ticker
        + "_"
        + str(date)
        + ".png"
    ...
    plt.savefig(
        "Investigations/ARMA_model/Diagnostics_"
        + asset.ticker
        + "_"
        + str(date)
        + ".png",
