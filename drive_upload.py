...
def upload(file_path_local: str, parent_directory_remote: str, file_path_remote: str):
    ...
    parent_directory_id = get_folder_id(
        drive_service, parent_directory_remote, parent_id=dissertation_folder_id
    )
...
def upload_class_files():
    ...
    class_files = [
        "Collection.py",
        "AssetCollection.py",
        "MacroEconomicCollection.py",
        "hyperparameters.py",
        "PortfolioCollection.py",
        "Asset.py",
        "PortfolioEnv.py",
    ]
...
def download_folder(folder_name: str):
    ...
    drive_service = build("drive", "v3", credentials=creds)
    dissertation_folder_id = get_folder_id(drive_service, "Dissertation")
    models_folder_id = get_folder_id(
        drive_service, folder_name, parent_id=dissertation_folder_id
    )

    results = (
        drive_service.files()
        .list(
            q=f"'{models_folder_id}' in parents",
            spaces="drive",
...
def download_logs():
    ...
    drive_service = build("drive", "v3", credentials=creds)
    dissertation_folder_id = get_folder_id(drive_service, "Dissertation")
    logs_folder_id = get_folder_id(
        drive_service, "Logs", parent_id=dissertation_folder_id
    )

    results = (
        drive_service.files()
        .list(
            q=f"'{logs_folder_id}' in parents", spaces="drive", fields="files(id, name)"
        )
        .execute()
    )
