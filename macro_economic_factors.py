...
def generate_macro_economic_factors():
    ...
    """
    macro_economic_factors_series_ids = [
        "GDP",
        "UNRATE",
        "FEDFUNDS",
        "CPIAUCNS",
    ...
    for series_id in macro_economic_factors_series_ids:
        ...
        asset = Asset(
            series_id, index, values, open_list=None, close_list=None, volume_list=None
        )
        macro_economic_factors_list[series_id] = asset

    index, values = extract_snp500()
    asset = Asset(
        "SP500", index, values, open_list=None, close_list=None, volume_list=None
    )
    macro_economic_factors_list["SP500"] = asset

    index, values = extract_nasdaq()
    asset = Asset(
        "NASDAQ", index, values, open_list=None, close_list=None, volume_list=None
    )
